...
|Accessing low level feedback sensor. |  getPulseWidthPosition() getEncPosition() getEncVelocity() getAnalogInVelocity()  | getSensorCollection().getPulseWidthPosition() getSensorCollection().getQuadraturePosition() getSensorCollection().getQuadratureVelocity() getSensorCollection().getAnalogInVel()| Low level functions that access sensor data directly are now tucked under the sensor collection object.
...
|**Motor/Sensor Direction**|reverseSensor / reverseOutput / setInvert / SetSensorDirection / setClosedLoopOutputDirection / (similar reverse functions)|talon.SetInvert / talon.SetSensorPhase| Use sensor phase to align positive sensor velocity to positive motor output.  Once this is done you will never have to call any reverse function for the purpose of alignment.  Use SetInvert to choose what motor direction to apply when positive output is applied (green LEDs)|
|**Sensor Units**|
...
|Velocity Measurement| SetVelocityMeasurementPeriod()/  SetVelocityMeasurementWindow()| configVelocityMeasurementPeriod()/ configVelocityMeasurementWindow()| Functions renamed to config.|
