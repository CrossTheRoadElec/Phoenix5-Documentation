...
In other words, a Talon or Victor can **execute closed-loop modes with sensor values sourced by other Talons, CANifiers, or Pigeon IMUs** on the same CAN-bus.
...
- Sensor is physically located too far from the motor controller for reliable/robust wiring.
...
In order to use remote sensors, **the sensor must first be validated by the device that it's connected to**.
**If the sensor is not reliable at the hosted device, then it definitely will not function when utilized remotely.**
